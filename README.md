...
190 tests, happy/edge/failure/concurrent path per module — most against
...
**Visual** (renders on GitHub — two independent extraction paths, nothing in
either depends on the other until the cross-reference merge at the bottom):

```mermaid
flowchart TB
    subgraph PID["P&ID path"]
        A["diagram.pdf"] --> B["pdf_to_image.py"]
        B --> C1["shape_detection.py (raster, always on)"]
        B --> C2["vector_*.py (opt-in / auto-fallback)"]
        B --> C3["yolo_detector.py (--use-yolo)"]
        C1 --> D["merged DetectedShape list"]
        C2 --> D
        C3 --> D
        D --> E["ocr_tagging.py (+ llm_ocr_assist.py --llm-ocr-assist)"]
        E --> F["connector_detection.py"]
        F --> G["graph_builder.py"]
        G --> H["NetworkX graph, merged across sheets"]
        B --> TBK["title_block.py"]
    end
    subgraph SOP["SOP path"]
        S["sop.docx"] --> P["docx_parser.py"]
        P --> T["tag_extractor.py (narrative)"]
        P --> U["limits_table.py (design-limits table)"]
    end
    H --> X1["crossref/compare.py"]
    T --> X1
    TBK --> X2["crossref/limit_check.py"]
    U --> X2
    X1 --> R["crossref/report.py — merge + severity rank"]
    X2 --> R
    R --> OUT["output/: cross_reference_report.json + .log, pid_graph.json + .graphml"]
```

Two design decisions the diagram doesn't show, worth knowing before reading
the code:

- **`component_types.py` has two coarse-category collapse functions, deliberately not merged.**
  `coarse_category()` (6 categories) is what `crossref/compare.py` compares
  against the SOP's declared-type vocabulary; `project_coarse_category()` (3
  categories: valve/instrument/equipment) is what the `evaluation/` scripts
  compare against this project's own ground truth. They diverge on purpose —
  e.g. `tank` stays `"tank"` in one, collapses to `"equipment"` in the
  other — so a "simplification" that merges them would silently break one of
  the two comparisons.
- **Every opt-in flag (`--llm-ocr-assist`, `--use-yolo`, `--symbol-type-assist`,
  `--equipment-label-discovery`) is strictly additive.** Each only fills a gap
  a cheaper/deterministic path left unresolved; none ever overrides a value
  already resolved by a higher-confidence path. Turning a flag off can only
  ever mean *less* filled in, never a different answer for something already
  resolved.
...
**Reconfirmed after a later fix.** A post-submission bug-remediation pass found
`extract_title_block_limits` had two real bugs: the tag/clause positional-pairing
fallback silently reused the first clause's value for any tag past the matched
clause count instead of leaving it unresolved, and `known_prefixes` was a plain
`{prefix: tag}` dict that would silently drop every known tag but one whenever
two known tags shared a prefix letter. Neither happens to trigger on this
document's real 4-tag/4-clause title blocks (each sheet's tags all have distinct
prefixes and every tag's clause OCR'd cleanly), so the result above was never
wrong — but both were fixed (regression-tested in `test_title_block.py`) and the
full pipeline was re-run against the real PDF/SOP afterward to confirm: still
**4 of 4 tags matched, 0 `DESIGN_LIMIT_MISMATCH` findings.**
...
| Valve coverage | 6/29 (20.7%) |
| Equipment coverage | 2/2 (100%) |

**Correction, found during a post-submission bug-remediation pass:** this table
previously read 10/29 (34.5%) for valve coverage. Re-running
`score_cv_accuracy.py --page 0` directly (no flags) during that session
measured 6/29 — checked against pre-fix code via `git stash` and confirmed the
discrepancy predates that session's fixes, so it isn't a regression any of
them caused; it's this table having gone stale at some earlier point without a
remeasurement to catch it. Root cause not chased down further (out of scope
for that pass — it was fixing four independently-found bugs, not auditing
every historical number in this file), corrected to the actual current,
reproducible value instead of left standing. Instrument and equipment
coverage both remeasured unchanged. The YOLO-assisted numbers later in this
file (`--use-yolo`, "Retrained on a merged coarse-class dataset" and below)
were also reconfirmed during that pass and matched what's already documented.
...
### Post-submission bug-remediation pass: four real bugs found and fixed by independent review

A dedicated code-review pass (two independent agents, one over the core
crossref/SOP/pipeline deliverable, one over the CV/vector extraction modules)
found four real, previously-undocumented bugs, none caught by the Opus review
above since they're in different modules:

1. **`coarse_category()` in `component_types.py` only covered 11 of
   `TAG_PREFIX_TYPES`' ~44 fine-grained values**, identity-mapping the rest.
   A correctly-resolved `pressure_indicator` or `level_switch_high` never
   matched an SOP-declared `"instrument"`, spuriously firing `TYPE_MISMATCH`
   on a genuinely correct classification — the same bug class already found
   and fixed once for `score_cv_accuracy.py`'s own local category map (see
   "A real bug in how these numbers were measured" further up), just missed
   in this twin instance. Fixed by pattern-matching like
   `project_coarse_category()` already does, instead of an incomplete dict.
2. **`title_block.py`'s positional tag/clause pairing silently misattributed
   design limits** when OCR found fewer clauses than tags — falling back to
   tag[0]'s value instead of `None`. Fixed to leave the value honestly
   unresolved instead. Didn't trigger on the real document (clause count
   always matched tag count there), reconfirmed by re-running the full
   pipeline against the real PDF/SOP after the fix (see "Title-block
   cross-check" above — still 0 mismatches).
3. **`title_block.py`'s `known_prefixes` was a plain `{prefix: tag}` dict**,
   silently dropping every known tag but one whenever two known tags shared a
   prefix letter. Fixed to group by prefix so every tag sharing an ambiguous
   prefix surfaces (sharing the one clause OCR actually found, since a single
   header instance can't distinguish which physical tag it belongs to).
4. **`vector_symbols.py`'s `extract_circle_symbols` never checked a
   candidate path was actually closed**, despite its own module docstring
   claiming it targets "closed vector paths" — only bbox aspect ratio and
   size band were checked. Every sibling detector in the same package
   (`vector_valves.py`, `vector_noise.py`) checks path item composition
   before trusting a bbox; this one didn't. Fixed by requiring the path be an
   all-curve loop whose start and end points coincide (`get_drawings()`'s own
   `closePath` flag doesn't reflect this — verified `False` on every real
   instrument bubble on this document, since the final curve's endpoint
   already coincides with the first curve's start). Verified against the real
   document: all 6 real instrument bubbles on page 0 still match, zero
   regressions (`test_extract_circle_symbols_closure_check_preserves_real_count_regression`).

Also fixed as part of the same pass: a DRY duplication (`vector_symbols.py`,
`vector_valves.py`, and `vector_equipment.py` each reimplemented
`vector_lines.py`'s rotation/dpi pixel-space transform inline instead of
importing it — same risk class as the `weights_path` late-binding bug above,
now consolidated), and a thousands-separator gap in `limits_table.py`'s
`_to_float` (a comma-formatted pressure value like `"1,275"` silently became
`None` instead of `1275.0`).

Each fix has a regression test (`tests/test_component_types.py`,
`tests/test_crossref.py`, `tests/test_title_block.py`,
`tests/test_limits_table.py`, `tests/test_vector_extraction.py`). Full
suite: 190 passing, Qbital Test Quality Score unchanged at 81.0/100 (B) —
type and depth coverage were already maxed, and these are all
`authored_claude_sonnet`, so source diversity didn't move. The full pipeline
was re-run against the real PDF/SOP after all four fixes (see "Reconfirmed
after a later fix" above and the corrected valve-coverage table in "CV
accuracy scoring") to confirm no regression on real data, not just on the
synthetic fixture.
